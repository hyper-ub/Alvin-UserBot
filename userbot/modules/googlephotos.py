...
async def create_token_file(token_file, event):
    ...
    async with event.client.conversation(event.chat_id, timeout=600) as conv:
        await conv.send_message(
            "go to "
            "Link and follow it "
            f"your browser master: {authorize_url} and "
            "reply Kode"
        )
        ...
        await imp_gsem.reply(
            ...
            "= "
            f"<u>{imp_gsem.id}</u> ..!"
            "\n\n<i>This is just necessary, "
            "if you running Ephimeral File-System</i>.",
            parse_mode="html",
        )
        return storage
...
async def upload_google_photos(event):
    ...
    if not event.reply_to_msg_id and not input_str:
        await event.edit(
            "©️ <b>[ALVIN]</b>\nNbody help you", parse_mode="html"
        )
        return
    ...
    if not is_cred_exists:
        await event.edit("First Run <code>;gpsetup</code> master", parse_mode="html")
    ...
    if not file_path:
        await event.edit("<b>[STOP DOING SPAM]</b>", parse_mode="html")
        return

    file_name, mime_type, file_size = file_ops(file_path)
    await event.edit("File downloaded, " "Collecting Upload Information")

    async with aiohttp.ClientSession() as session:
        headers = {
            "Content-Length": "0",
            "Alvin-Goog-Upload-Command": "start",
            "Alvin-Goog-Upload-Content-Type": mime_type,
            "Alvin-Goog-Upload-File-Name": file_name,
            "Alvin-Goog-Upload-Protocol": "resumable",
            "Alvin-Goog-Upload-Raw-Size": str(file_size),
        ...
        real_upload_url = step_one_resp_headers.get("Alvin-Goog-Upload-URL")
        logger.info(real_upload_url)
        upload_granularity = int(
            step_one_resp_headers.get("Alvin-Goog-Upload-Chunk-Granularity")
        )
        ...
        async with aiofiles.open(file_path, mode="rb") as f_d:
            for i in range(number_of_req_s):
                ...
                headers = {
                    ...
                    "Alvin-Goog-Upload-Command": "upload",
                    "Alvin-Goog-Upload-Offset": str(offset),
                    "Authorization": "Bearer " + creds.access_token,
                }
                logger.info(i)
                logger.info(headers)
            ...
            headers = {
                ...
                "Alvin-Goog-Upload-Command": "upload, finalize",
                "Alvin-Goog-Upload-Offset": str(number_of_req_s * upload_granularity),
                "Authorization": "Bearer " + creds.access_token,
            }
            logger.info(headers)
            response = await session.post(
    ...
    await event.edit("uploading google photo, " "getting FILE URI`")
    ...
    try:
        ...
        await event.edit(f"`[SUCCESFULLY]`\n\nupload photo to google [click to look]({photo_url})")
