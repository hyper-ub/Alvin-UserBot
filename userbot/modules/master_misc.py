...
async def _(event):
    ...
    if c.admin_rights or c.creator:
        ...
        await event.edit("master, Your needs Admin permission to perform this command")
...
async def _(event):
    ...
    q = 0
    r = 0
    await event.edit("_Looking for Participant List...._")
...
async def _(event):
    ...
    await event.edit("*Rename And Upload In The Process, It Will Take A Long Time If The File Size Is Large")
    input_str = event.pattern_match.group(1)
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await event.edit("donwloading in {} second. uploading in {} second.".format(ms_one, ms_two))
        else:
            await event.edit("File not found{}".format(input_str))
    else:
        await event.edit("_master, please reply to media_")
...
async def potocmd(event):
    ...
    if id.strip() == "":
        ...
        try:
            id = int(id)
            if id <= 0:
                await event.edit("The ID Number You Entered is Invalid")
                return
        ...
        if int(id) <= (len(photos)):
            ...
            await event.edit("Can't Find This User's Photo")
            return
...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit("Please Reply To Link.")
        return
    reply_message = await event.get_reply_message()
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.reply("please don't block @CheckRestrictionsBot")
            return
        if response.text.startswith(""):
            await event.edit("There is an error")
        else:
            await event.delete()
...
async def weebify(event):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await event.edit("master, What Text Should I Weebify?")
        return
...
async def thicc(bolded):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await bolded.edit("master, What Text Should I bold ??")
        return
    string = ''.join(args).lower()
    for normiecharacter in string:
        if normiecharacter in normiefont:
...
async def mediv(medievalx):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await medievalx.edit("master, What Text Should I medibold??")
        return
    string = ''.join(args).lower()
    for normiecharacter in string:
        if normiecharacter in normiefont:
...
async def doublex(doublestrucktx):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await doublestrucktx.edit("master, What Text Should I double struck??")
        return
    string = ''.join(args).lower()
    for normiecharacter in string:
        if normiecharacter in normiefont:
...
async def cursive2(cursivebolded):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await cursivebolded.edit("master, What Text Should I curbold??")
        return
    string = ''.join(args).lower()
    for normiecharacter in string:
        if normiecharacter in normiefont:
...
async def medival22(medivallite):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await medivallite.edit("master, What Text Should I medi?")
        return
    string = ''.join(args).lower()
    for normiecharacter in string:
        if normiecharacter in normiefont:
...
async def xcursive(cursivelite):
    ...
    if not args:
        ...
        args = get.text
    if not args:
        await cursivelite.edit("master, What Text Should I cursive?")
        return
    string = ''.join(args).lower()
    for normiecharacter in string:
        if normiecharacter in normiefont:
...
async def _(event):
    ...
    await event.edit("Successfully Restore Your Account ツ")

CMD_HELP.update({
    "misc":
    ";calc\
\nExplanation: .calc <term1><operator><term2>\nFor eg .calc 02*02 Or 99*99 (Significant Zero) (Minimum Two Term And Two Digits).\
\n\n;remove\
\nExplanation: use in group .remove d or y or m or w or o or q or r.\n(d=Deleted Accounts y=userstatsempty m=userstatsmonth w=userstatsweek o=userstatsoffline q=userstatsonline r=userstatsrecently).\
\n\n;xcd\
\nExplanation: Type xcd <query>.ps:I'm So Bored:v\
\n\n;rnupload filename.extension\
\nExplanation: Reply To Sticker And Type .rnupload xyz.jpg\
\n\n;res\
\nExplanation: Type Account Username, Channel, Group Or Bot Together .res And Check Restrictions\
\n\nw;atch <movie/tv>\
\nExplanation: Knowing the Details About Film.\
\n\n;web <text>\
\nExplanation: Weebify Text\
\n\n(;bold <Text>,;cur <Text>,;curbold <Text>,m;edi <Text>,;medibold <Text>,;doublestruck <Text>)\
\nExplanation: Make Text <Bold,Cursive,Cursivebold,Medival,Medivalbold,Gayishbold>"})
CMD_HELP.update({"app": "**Modules:** __App__\n\n**Command:** ;app <app name>"
                 "\n**Explanation:** Get Detailed Application Information"})
CMD_HELP.update({"undelete": "**Modules:** __Undelete__\n\n**Command:** ;undlt"
                 "\n**Explanation:** Undo Deleted Message But You Must Be Admin"})
CMD_HELP.update({"grab": "**Modules:** __Grab__\n\n**Command:** ;grab <amount>"
                 "\n**Explanation:** Taking Profile, Can Reply To Another User's Message To Retrieve His Profile\n\n**WARN:** DON'T ABUSE THIS COMMAND"})
CMD_HELP.update(
    {
        "clone": "**Modules:** __Clone__\n\n**Command:** ;clone <username/reply message>"
        "\n**Explanation:** Clone Someone's Account, But Username Is Not Clone"
        "\n\n**Command:** ;rclone"
        "\n**Explanation:** Restore Your Account As Before"})
CMD_HELP.update({"glitch": "**Modules:** __Glitch__\n\n**Command:** ;glitch <level>"
                 "\n**Explanation:** Reply To Media, Gives Glicth Effect To Media, Level 1 - 8"
                 "\n\n**Command:** glitchs <level>"
                 "\n**Explanation:** Reply To Media, Gives Glitch Effect To Media, Level 1 - 8\n\n**NOTE:** If You Don't Provide Level Then Automatically Default Choose Level 2"})
CMD_HELP.update({"randomprofile": "**Modules:** __Random Profile__\n\n**Command:** ;randompp"
                 "\n**Explanation:** Change Your Profile Photo Automatically, To Stop This Command Use Restart Command"})
