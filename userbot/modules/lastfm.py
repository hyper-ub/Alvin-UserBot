...
LFM_BIO_RUNNING = "````master, last.fm current music to bio already running.```"
LFM_BIO_ERR = "```No options specified.``` "
LFM_LOG_ENABLED = "```last.fm logging to bot logging is now enabled.``` "
LFM_LOG_DISABLED = "```last.fm logging to bot logging is now disabled.``` "
LFM_LOG_ERR = "```No options specified.```"
ERROR_MSG = "```master, modul last.fm stopped, got an unexpected error.``` "
...
async def last_fm(lastFM):
    ...
    await lastFM.edit("_processing..._")
    preview = None
    ...
    if playing is not None:
        ...
        if image:
            output = f"[‎]({image})[{LASTFM_USERNAME}]({username}) __now listening:__\n\n• [{playing}]({rectrack})\n`{tags}`"
            preview = True
        else:
            output = f"[{LASTFM_USERNAME}]({username}) __now listening:__\n\n• [{playing}]({rectrack})\n`{tags}`"
    else:
        recent = User(LASTFM_USERNAME, lastfm).get_recent_tracks(limit=3)
        playing = User(LASTFM_USERNAME, lastfm).get_now_playing()
        output = f"[{LASTFM_USERNAME}]({username}) __lastes listening:__\n\n"
    ...
    while LASTFMCHECK:
        try:
                ...
                try:
                    if BOTLOG and LastLog:
                        await bot.send_message(
                            BOTLOG_CHATID, f"trying change bio to\n{lfmbio}"
                        )
            ...
            try:
                ...
                if BOTLOG and LastLog:
                    await bot.send_message(BOTLOG_CHATID, f"An error occurred while changing the bio:\n{err}")
        except FloodWaitError as err:
            if BOTLOG and LastLog:
                await bot.send_message(BOTLOG_CHATID, f"An error occurred while changing the bio:\:\n{err}")
        except WSError as err:
...
CMD_HELP.update(
    {
        "lastfm": ";lastfm\
    \nexplanation: Shows the current scrobbling track or the most recent scrobbles if nothing is playing.\
    \n\nlastbio: ;lastbio <on/off>\
    \nexplanation: Enable/Disable current last.fm playback to bio.\
    \n\nlastlog: ;lastlog <on/off>\
    \nexplanation: Enable/Disable last.fm bio log in bot-log group."
    }
)
