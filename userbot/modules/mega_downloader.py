...
async def subprocess_run(megadl, cmd):
    ...
    if exitCode != 0:
        await megadl.edit(
            "**Error detected while running subprocess.**\n"
            f"exitCode : `{exitCode}`\n"
...
async def mega_downloader(megadl):
    await megadl.edit("getting information...")
    ...
    if link:
        ...
        return await megadl.edit("use: ;mega **<MEGA.nz link>**")
    try:
        ...
        if "file" in link:
            ...
            await megadl.edit("support deleted folder download...")
            return
    except IndexError:
    ...
    if downloader.isSuccessful():
        ...
        try:
            ...
            await megadl.edit(
                f"`{file_name}`\n\n"
                f"download succesfully in: `{file_path}`.\n"
                f"Download took: {time_formatter(download_time)}."
            )
            return None
    else:
        await megadl.edit(
            "download failed, " "check heroku log for more details."
        )
...
CMD_HELP.update(
    {
        "mega": "**Modules:** __Mega__\n\n**command:** ;mega <MEGA.nz link>"
        "\n**explanation:** Reply to MEGA.nz link or paste your MEGA.nz link "
        "to download files to your userbot server."
    }
)
