...
async def glitch(event):
    if not event.reply_to_msg_id:
        await event.edit("i wanna glitch some ghost")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.media:
        await event.edit("reply to image/Sticker")
        return
    ...
    if event.is_reply:
        ...
        if isinstance(data, bool):
            await event.edit("file is not supported...")
            return
    else:
    ...
    try:
        ...
        value = 2
    await event.edit("doing glitch for this media ツ")
    ...
    await event.edit("Uploading Glitch Media ツ")
    c_time = time.time()
...
CMD_HELP.update(
    {
        "glitch": ">;glitch <1-8>"
        "\nUsage: reply to Sticker/image.\nGlitch Level 1-8 If you don't create a level, it will automatically default to level 2"
    }
)
