...
if 1 == 1:
    strings = {
        ...
        "cannot_send_stickers": "`cannot end message in this chat.`",
        "admin": "admin",
        "creator": "creator",
        "hidden": "hidden",
        "channel": "Channel"}
...
async def quotess(qotli):
    ...
    if not qotli.reply_to_msg_id:
        return await qotli.edit("_please reply to message master_")
    ...
    if reply_message.sender.bot:
        return await qotli.edit("_please reply to message master_")
    await qotli.edit("_processing sticker, please wait ツ_")
    try:
        async with bot.conversation(chat) as conv:
            try:
                ...
                return await qotli.reply("*please don't Block @QuotLyBot open block and try again*")
            if response.text.startswith("Hi!"):
                await qotli.edit("*please enable your private forward settings*")
            else:
                await qotli.delete()
...
CMD_HELP.update({
    "quotly":
    ";q\
\nUsage: creat your message to sticker.\
\n\n;xquote\
\nUsage: creat your message to sticker."
})
