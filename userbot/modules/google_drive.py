...
if __ is not None:
    if "uc?id=" in G_DRIVE_FOLDER_ID:
        LOGS.info("G_DRIVE_FOLDER_ID invalid url folder...")
                ...
                try:
                    ...
                    if "http://" not in __ or "https://" not in __:
                        ...
                        if True in [_1 or _2]:
                            ...
                            LOGS.info("G_DRIVE_FOLDER_ID "" invalid ID...")
                            G_DRIVE_FOLDER_ID = None
                    else:
                        LOGS.info("G_DRIVE_FOLDER_ID "" invalid ID...")
                        G_DRIVE_FOLDER_ID = None
...
async def generate_credentials(gdrive):
    ...
    if helper.get_credentials(str(gdrive.from_id)) is not None:
        await gdrive.edit("_You have authorized the token..._")
    ...
    if G_DRIVE_DATA is not None:
        try:
            ...
            await gdrive.edit(
                "`[AUTENTICATION - ERROR]`\n\n"
                "`Status` : **BAD**\n"
                "`reason` : **G_DRIVE_DATA** invalid identity!"
            )
            return False
    else:
        """ - Only for old user - """
        if G_DRIVE_CLIENT_ID is None and G_DRIVE_CLIENT_SECRET is None:
            await gdrive.edit(
                "`[AUTENTICATION - ERROR]`\n\n"
                "`Status` : **BAD**\n"
                "`reason` : master, please take **G_DRIVE_DATA** "
    ...
    await gdrive.edit("_Creating credentials..._")
    flow = InstalledAppFlow.from_client_config(
        configs, SCOPES, redirect_uri=REDIRECT_URI
    )
    auth_url, _ = flow.authorization_url(access_type="offline", prompt="consent")
    msg = await gdrive.respond("_Go to your BOTLOG group to authenticate token..._")
...
async def create_app(gdrive):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            await gdrive.edit("_Refreshing credentials..._")
            """ - Refresh credentials - """
            ...
            await gdrive.edit("*Empty credentials, please create...*")
            return False
...
async def reset_credentials(gdrive):
    ...
    await gdrive.edit("rearrenge information...")
    helper.clear_credentials(str(gdrive.from_id))
    await gdrive.edit("finish...")
...
async def download(gdrive, service, uri=None):
    ...
    if uri:
        ...
        try:
            ...
            reply += (
                "`[FILE - CANCELED]`\n\n"
                "`Status` : **OK** - received signal canceled."
            )
            return reply
        else:
        ...
        if isfile(required_file_name):
            try:
                ...
                reply += (
                    ...
                    "`Status :` **OK** - Uploaded successfully.\n\n"
                )
                return reply
        else:
...
async def download_gdrive(gdrive, service, uri):
    ...
    try:
        ...
        if mimeType == "application/vnd.google-apps.folder":
            await gdrive.edit("_canceled, folder download not supported..._")
            return False
        file_path = TEMP_DOWNLOAD_DIRECTORY + file_name
    ...
    await gdrive.edit(
        ...
        f"`Path    :` `{file_path}`\n"
        "`Status    :` **OK** - download succesfully."
    )
    msg = await gdrive.respond("_JAnswer questions in your BOTLOG group_")
    async with gdrive.client.conversation(BOTLOG_CHATID) as conv:
        ask = await conv.send_message("_continue with mirroring? [y/N]_")
        try:
            r = conv.wait_event(events.NewMessage(outgoing=True, chats=BOTLOG_CHATID))
            r = await r
        except Exception:
    ...
    if ans.capitalize() == "N":
        ...
        await gdrive.client.send_message(
            BOTLOG_CHATID, "*Invalid answer type, just[Y/N]...*"
        )
        return reply
...
async def upload(gdrive, service, file_path, file_name, mimeType):
    try:
        await gdrive.edit("uploading in process...")
    except Exception:
        pass
    body = {
        "name": file_name,
        "description": "uploading from telegram with Alvin-Userbot.",
        "mimeType": mimeType,
    }
    try:
        if parent_Id is not None:
...
async def lists(gdrive):
    ...
    if query == "":
        query = "Not specified"
    if len(message) > 4096:
        await gdrive.edit("_Result too big, sent with file..._")
        with open("result.txt", "w") as r:
            r.write(f"Google Drive Query:\n{query}\n\nHasil\n\n{message}")
        await gdrive.client.send_file(
            gdrive.chat_id, "result.txt", caption="list Query Google Drive."
        )
    else:
        await gdrive.edit(
            "**Google Drive Query**:\n" f"`{query}`\n\n**result**\n\n{message}"
...
async def google_drive_managers(gdrive):
    ...
    await gdrive.edit("_sending information..._")
    service = await create_app(gdrive)
    if service is False:
        return None
...
async def cancel_process(gdrive):
    ...
    await gdrive.edit("_canceling..._")
    if len(downloads) != 0:
...
async def google_drive(gdrive):
    ...
    if not value and not gdrive.reply_to_msg_id:
        ...
        await gdrive.edit(
            "`[UNKNOWN - ERROR]`\n\n"
            "`Status` : **FAIL**\n"
            "`reason` : Confused about uploading files or messages/media being replied to."
        )
        return None
    service = await create_app(gdrive)
    if service is False:
    ...
    if isfile(value):
        ...
        if not uri and not gdrive.reply_to_msg_id:
            await gdrive.edit(
                "`[VALUE - ERROR]`\n\n"
                "`Status` : **BAD**\n"
                "`reason` : the value given is not a URL or file/folder path. "
                "If you think this is wrong, maybe you are using ;gd over and over"
                "file/folder value, for example `;gd <filename1> <filename2>` "
                "to upload from this file/folder path doesn't support it."
            )
            return False
...
async def set_upload_folder(gdrive):
    ...
    if exe == "rm":
        if G_DRIVE_FOLDER_ID is not None:
            ...
            await gdrive.edit(
                "`[FOLDER - SET]`\n\n"
                "`Status` : **OK** - use `G_DRIVE_FOLDER_ID` now."
            )
            return None
        else:
            try:
    ...
    try:
        ...
        if True in [c1 or c2]:
            parent_Id = inp
            await gdrive.edit(
                "`[PARENT - FOLDER]`\n\n" "`Status` : **OK** - Changed successfully."
            )
            return None
        else:
            await gdrive.edit(
                "`[PARENT - FOLDER]`\n\n" "`Status` : **ATTENTION** - forced use..."
            )
            parent_Id = inp
    else:
        if "uc?id=" in ext_id:
            await gdrive.edit(
                "`[URL - ERROR]`\n\n" "`Status` : **BAD** - invalid folderURL."
            )
            return None
        try:
            ...
            if " depth exceeded" in str(e):
                ...
                try:
                    await gdrive.edit(
                        ...
                        f"`{file.name}`\n"
                        "`Status` : **gagal**\n"
                        "`reason` : Download auto cancel, URI / Torrents off."
                    )
                except Exception:
                    pass
...
CMD_HELP.update(
    {
        "googledrive": "**Modules:** __Gdrive__\n\n**command:** ;gdauth"
        "\n**explanation:** get token to activate all google drive cmd services."
        "\nThis only needs to be run once in a lifetime."
        "\n\n**command:** ;gdreset"
        "\n**explanation:** reset your token if something bad happens or change the drive account."
        "\n\n**command:** ;gd"
        "\n**explanation:** Upload files from local or uri/url/drivelink to google drive."
        "\nFor the drivelink it is uploaded only if you want."
        "\n\n**command:** ;gdabort"
        "\n**explanation:** Cancel upload or download."
        "\n\n**command:** ;gdlist"
        "\n**explanation:** Get a list of folders and files with a default size of 50."
        "\nuse flags -l range [1-1000] for output limit."
        "\nuse flags -p parents-folder_id for a given list of folders in gdrive."
        "\n\n**command:** ;gdf mkdir"
        "\n**explanation:** Create a gdrive folder."
        "\n\n**command:** ;gdf check"
        "\n**explanation:** Check files/folders on gdrive."
        "\n\n**command:** ;gdf rm"
        "\n**explanation:** Delete files/folders on gdrive."
        "\nCannot be undone, this method skips junk files, so be careful..."
        "\n\n**command:** ;gdfset put"
        "\n**explanation:** Change upload directory on gdrive."
        "\n\n**command:** ;gdfset rm"
        "\n**explanation:** remove set parentId from cmd\n.gdfset put "
        "into **G_DRIVE_FOLDER_ID** and if upload is empty it will go to root."
        "\n\n**NOTES:**"
        "\nFor .gdlist you can combine the -l and -p flags with or without names "
        "at the same time, this must be the ` -l` flag first before using the `-p` flag.\n"
        "And by default a list of the latest 'modifiedTime' and then folders."
    }
)
