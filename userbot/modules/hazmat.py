...
async def _(hazmat):
    await hazmat.edit("_master Wants To Activate Hazmat Command ヅ_")
    level = hazmat.pattern_match.group(2)
    if hazmat.fwd_from:
        return
    if not hazmat.reply_to_msg_id:
        await hazmat.edit("_master,please reply to Sticker/image_")
        return
    reply_message = await hazmat.get_reply_message()
    if not reply_message.media:
        await hazmat.edit("_Words Can Destroy Anything master ヅ_")
        return
    chat = "@hazmat_suit_bot"
    await hazmat.edit("_Hazmat Command Activated, Processing.... _")
    ...
    async with hazmat.client.conversation(chat) as conv:
        try:
            ...
            await hazmat.reply("master, please unblock @hazmat_suit_bot and try again")
            return
        if response.text.startswith("I can't"):
            await hazmat.edit("_Sorry master, GIF Cannot...._")
