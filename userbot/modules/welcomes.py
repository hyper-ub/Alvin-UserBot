    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            await event.client.send_message(
                ...
                "\nmaster Installed a Welcome Group Command Message, This Is a Welcome Message Note "
                "Please Don't Erase master!"
            )
            ...
            return await event.edit(
                "To make media as Welcome message, BOTLOG_CHATID Must be set."
            )
    elif event.reply_to_msg_id and not string:
        rep_msg = await event.get_reply_message()
        string = rep_msg.text
    success = "Successfully Saved Welcome Message {} ツ"
...
async def show_welcome(event):
    ...
    cws = get_current_welcome_settings(event.chat_id)
    if not cws:
        return await event.edit("Here There Is No Welcome Message You Saved Lord ツ")
    elif cws and cws.f_mesg_id:
        msg_o = await event.client.get_messages(entity=BOTLOG_CHATID,
                                                ids=int(cws.f_mesg_id))
        await event.edit(
            "You have created a welcome message here ツ")
...
@register(outgoing=True, pattern="^;resetwelcome$")
async def del_welcome(event):
    ...
    if rm_welcome_setting(event.chat_id) is True:
        await event.edit("Deleting Welcome Message Successfully Done ツ")
    else:
        await event.edit("You Don't Save Any Welcome Message Here master ツ")


CMD_HELP.update({
    "welcome":
    ">;setwelcome <welcome message> or reply to message type ;setwelcome"
    "\nUsage: Saves the group welcome message."
    "\n\nWelcome Ordered Variable Formats are welcome:"
    "\n`{mention}, {title}, {count}, {first}, {last}, {fullname},"
    "{userid}, {username}, {my_first}, {my_fullname}, {my_last},"
    "{my_mention}, {my_username}`"
    "\n\n>;welcome"
    "\nUsage: Check the welcome message you saved."
    "\n\n>;reset welcome"
    "\nUsage: Delete the welcome message you saved."
})
