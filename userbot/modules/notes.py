...
async def remove_notes(clr):
    ...
    try:
        ...
        return await clr.edit("running in Non-SQL mode")
    notename = clr.pattern_match.group(1)
    if rm_note(clr.chat_id, notename) is False:
        return await clr.edit("Cannot find notes: **{}**".format(notename))
    else:
        return await clr.edit(
            "Note deleted successfully: **{}**".format(notename))
...
async def add_note(fltr):
    ...
    try:
        ...
        return await fltr.edit("running in Non-SQL mode")
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            await fltr.client.send_message(
                ...
                "\n\nThe following messages are saved as record reply data for chat, DO NOT delete !!"
            )
            ...
            return await fltr.edit(
                "Saving media as data for records requires setting BOTLOG_CHATID."
            )
...
async def kick_marie_notes(kick):
    ...
    if bot_type not in ["marie", "rose"]:
        return await kick.edit("that bot is not supported")
    await kick.edit("*Will kick all Notes!*")
    await sleep(3)
    ...
    for i in filters:
        ...
        await sleep(0.3)
    await kick.respond(
        "*successfully cleared bot records yaay!*\n give me a cake!")
    if BOTLOG:
        await kick.client.send_message(
            BOTLOG_CHATID, "I cleared all Notes in " + str(kick.chat_id))
...
CMD_HELP.update({
    ...
    "\
#<notename>\
\nUsage: Gets the specified record.\
\n\n;save <notename> <notedata> or reply to messages with .save <notename>\
\nUsage: Save the replied message as a note with notename. (Works with photos, documents and stickers for you!)\
\n\n;notes\
\nUsage: Gets all saved notes in a chat.\
\n\n;clear <notename>\
\nUsage: Delete the specified note.\
