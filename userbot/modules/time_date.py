...
async def time_func(tdata):
    """ Untuk perintah .time, kembalikan waktu
        1. State passed as argument,
        2. Default bot user country (set using .settime),
        3. Server on which to run user bots.
    """
...
async def date_func(dat):
    ...
    if len(timezones) == 1:
        ...
        if tz_num:
            ...
            return_str = f"`{c_name} have multiple time zones:`\n"

            for i, item in enumerate(timezones):
                return_str += f"`{i+1}. {item}`\n"

            return_str += "\nChoose one by typing the number "
            return_str += "in command.\n"
...
\nUsage: Usage: Get the time of a country. If a country has multiple time zones, it will list them all and let you choose one.\
\n\n;date <country name/code> <timezone number>\
\nUsage: Get the date of a country. If a country has multiple time zones, it will list them all and let you choose one."
})
